...
public class BackKnee : MonoBehaviour
{
    public TextMeshProUGUI BehideKneeText;
    public TextMeshProUGUI type_counting;
    public TextMeshProUGUI TimeText;
    public TextMeshProUGUI switcherText;
    public TextMeshProUGUI ScoreNumText;
    public float updateInterval = 1f;
    public static int range5 = 30;
    public int other_range5 = 10;
    ...
    void Update()
    {
        if (Stand.action == 2)
        {
            if (Stand.type == 4)
            {
                ...
                if (Math.Abs(RKnee10.transform.localPosition.y - LFoot14.transform.localPosition.y) <= range5)
                {
                    ClearTimer.stop1 = false;
                    BehideKneeText.text = "Correct!";
                    if (ClearTimer.takeTime >= 3)
                    {
                        ClearTimer.stop1 = true;
                        ClearTimer.InitTime();
                        BehideKneeText.transform.localPosition = new Vector3(200, -250, 0);
                        BehideKneeText.text = "Finished!";
            ...
            if (Stand.switcher <= 2)
            {
                if (Math.Abs(RKnee10.transform.localPosition.y - LFoot14.transform.localPosition.y) <= other_range5)
                {
                    BehideKneeText.text = " ";
                }
                else
                {
                    BehideKneeText.text = "BackKnee Wrong!";
                }
            }
            ...
            if (Stand.type == 6)
            {
                ScoreNumText.text = Stand.score.ToString();
                ClearTimer.StartTimeCounting();
                if (Math.Abs(LKnee13.transform.localPosition.y - RFoot11.transform.localPosition.y) <= range5)
                {
                    ClearTimer.stop1 = false;
                    BehideKneeText.text = "Correct!";
            ...
            if (Stand.switcher <= 1)
            {
                if (Math.Abs(LKnee13.transform.localPosition.y - RFoot11.transform.localPosition.y) <= other_range5)
                {
                    BehideKneeText.text = " ";
                }
                else
